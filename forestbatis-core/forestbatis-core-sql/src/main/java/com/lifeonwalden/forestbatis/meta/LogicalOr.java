 ...
 * @param <T>
 */
public class LogicalOr<T> extends AbstractQueryNode<T> {
    // 逻辑或计算结果
     ...
     * @param result 计算结果
     */
    public LogicalOr(ColumnMeta column, PropertyMeta factor, PropertyMeta result) {
     ...
     */
    public LogicalOr(ColumnMeta column, PropertyMeta factor, PropertyMeta result, Function<Optional<T>, Boolean> enableCheck) {
        this.column = column;
        this.compareRelation = NodeRelation.LOGICAL_OR;
