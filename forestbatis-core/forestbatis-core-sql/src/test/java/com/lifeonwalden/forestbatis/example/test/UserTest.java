...
public class UserTest {
    ...
    public void base_withPickedColumn() {
        String sql = UserBuilder.SELECT.overrideReturnColumn(Arrays.asList(UserTableInfo.Id, UserTableInfo.Birthday)).build().getSql();
        Assert.assertTrue(sql, "select id, birthday from User".equals(sql));
    }

    @Test
    public void base_withExcludeColumn() {
        String sql = UserBuilder.SELECT.excludeReturnColumn(Arrays.asList(UserTableInfo.Name)).build().getSql();
    ...
    public void base_withAddColumn() {
        String sql = UserBuilder.SELECT.excludeReturnColumn(Arrays.asList(UserTableInfo.Name, UserTableInfo.Age, UserTableInfo.Birthday))
                .addReturnColumn(Arrays.asList(UserTableInfo.Birthday)).build().getSql();
        Assert.assertTrue(sql, "select id, birthday from User".equals(sql));
    }

    @Test
    public void base_withOrder() {
        String sql = UserBuilder.SELECT.overrideOrder(Arrays.asList(new Order(UserTableInfo.Birthday))).build().getSql();
    ...
    public void base_withIsNull() {
        String sql = UserBuilder.SELECT.overrideQuery(new IsNull(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where age is null ".equals(sql));
    }

    @Test
    public void base_withIsNotNull() {
        String sql = UserBuilder.SELECT.overrideQuery(new IsNotNull(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where age is not null ".equals(sql));
    }

    @Test
    public void base_withLike() {
        String sql = UserBuilder.SELECT.overrideQuery(new Like(UserTableInfo.Name)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where name like ?".equals(sql));
    }

    @Test
    public void base_withNotLike() {
        String sql = UserBuilder.SELECT.overrideQuery(new NotLike(UserTableInfo.Name)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where name not like ?".equals(sql));
    }

    @Test
    public void base_withBt() {
        String sql = UserBuilder.SELECT.overrideQuery(new Bt(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where age > ?".equals(sql));
    }

    @Test
    public void base_withBte() {
        String sql = UserBuilder.SELECT.overrideQuery(new Bte(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where age >= ?".equals(sql));
    }

    @Test
    public void base_withLt() {
        String sql = UserBuilder.SELECT.overrideQuery(new Lt(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where age < ?".equals(sql));
    }

    @Test
    public void base_withLte() {
        String sql = UserBuilder.SELECT.overrideQuery(new Lte(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where age <= ?".equals(sql));
    }

    @Test
    public void base_withNeq() {
        String sql = UserBuilder.SELECT.overrideQuery(new Neq(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql, "select id, name, age, birthday from User where age <> ?".equals(sql));
    }

    @Test
    public void base_withCompound() {
        String sql = UserBuilder.SELECT.overrideQuery(new CompoundQuery(new Bt(UserTableInfo.Age).or(new Eq(UserTableInfo.Name))).or(new Eq(UserTableInfo.Birthday))).build().getSql();
    ...
    public void base_withCompoundDisableOne() {
        String sql = UserBuilder.SELECT.overrideQuery(
                new CompoundQuery(new Bt(UserTableInfo.Age).or(new Eq<User>(UserTableInfo.Name, user -> user.isPresent() && null != user.get().getName())))
                        .or(new Eq(UserTableInfo.Birthday))).build().getSql();
        Assert.assertTrue(sql,
    ...
    public void base_withExists() {
        String sql = UserBuilder.SELECT.overrideQuery(
                new Exists(new TempTable("account", new SubSelect()
                        ...
                        .setQuery(new Eq(UserTableInfo.Id, AccountTableInfo.UserId))))).build().getSql();
        Assert.assertTrue(sql,
    ...
    public void base_withInnerFirstDisabled() {
        String sql = UserBuilder.SELECT
                .overrideQuery(new Bt<User>(UserTableInfo.Age, user -> user.isPresent() && null != user.get().getAge()).and(new Eq(UserTableInfo.Name).or(new Eq(UserTableInfo.Birthday)))).build().getSql();
        Assert.assertTrue(sql,
    ...
    public void base_withInnerFirstTwoDisabled() {
        String sql = UserBuilder.SELECT
                .overrideQuery(new Bt<User>(UserTableInfo.Age, user -> user.isPresent() && null != user.get().getAge()).and(new Eq<User>(UserTableInfo.Name, user -> user.isPresent() && null != user.get().getName()).or(new Eq(UserTableInfo.Birthday)))).build().getSql();
        Assert.assertTrue(sql,
    ...
    public void base_withInnerSecondDisabled() {
        String sql = UserBuilder.SELECT
                .overrideQuery(new Bt(UserTableInfo.Age).and(new Eq<User>(UserTableInfo.Name, user -> user.isPresent() && null != user.get().getName()).or(new Eq(UserTableInfo.Birthday)))).build().getSql();
        Assert.assertTrue(sql,
                "select id, name, age, birthday from User where age > ? or birthday = ?".equals(sql));
    }
    ...
    public void base_withInNull() {
        String sql = UserBuilder.SELECT.overrideQuery(new In(UserTableInfo.Age)).build().getSql();
        Assert.assertTrue(sql,
    ...
    public void base_withInEmpty() {
        ...
        String sql = UserBuilder.SELECT.overrideQuery(new In(UserTableInfo.Sex)).build(user).getSql();
        Assert.assertTrue(sql,
                "select id, name, age, birthday from User".equals(sql));
    }
    ...
    public void base_withNotIn() {
        User user = new User();
        user.put(UserTableInfo.sex.getName(), Arrays.asList(1, 2));
        String sql = UserBuilder.SELECT.overrideQuery(new NotIn(UserTableInfo.Sex)).build(user).getSql();
        Assert.assertTrue(sql,
    ...
    @Test
    public void insert() {
        String sql = UserBuilder.INSERT.build().getSql();
        Assert.assertTrue(sql,
    ...
    public void insert_withNull() {
        String sql = UserBuilder.INSERT.build(new User()).getSql();
        Assert.assertTrue(sql,
                "insert into User (id, name, age, birthday) values(?, ?, ?, ?)".equals(sql));
    }
    ...
    public void insert_withNull_notInsertNull() {
        String sql = UserBuilder.INSERT.addInsertColumn(Arrays.asList(), false).build(new User().setId("id").setName("Tom")).getSql();
        Assert.assertTrue(sql,
    ...
    public void insert_withExcludeColumn() {
        String sql = UserBuilder.INSERT.excludeInsertColumn(Arrays.asList(UserTableInfo.Name)).build(new User().setId("id").setName("Tom")).getSql();
        Assert.assertTrue(sql,
    ...
    @Test
    public void delete() {
        String sql = UserBuilder.DELETE.build().getSql();
        Assert.assertTrue(sql,
    ...
    public void delete_withQuery() {
        String sql = UserBuilder.DELETE.overrideQuery(new Eq(UserTableInfo.Name)).build().getSql();
        Assert.assertTrue(sql,
    ...
    @Test
    public void update() {
        String sql = UserBuilder.UPDATE.build().getSql();
        Assert.assertTrue(sql,
    ...
    public void update_withNull() {
        String sql = UserBuilder.UPDATE.build(new User()).getSql();
        Assert.assertTrue(sql,
                "update User set name = ?, age = ?, birthday = ? where id = ?".equals(sql));
    }
    ...
    public void update_withNull_NotUpdateNull() {
        String sql = UserBuilder.UPDATE.addUpdateColumn(Arrays.asList(), false).build(new User().setId("id").setName("Tom")).getSql();
        Assert.assertTrue(sql,
    ...
    public void update_withQuery() {
        String sql = UserBuilder.UPDATE.overrideQuery(new Like(UserTableInfo.Name)).build(new User().setId("id").setName("Tom")).getSql();
        Assert.assertTrue(sql,
